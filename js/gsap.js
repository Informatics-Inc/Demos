...
// TILT FOCUS
  const container = document.querySelector(".mouse-listen");
  const tilt = document.querySelector(".bkg-texture");

  const rotX = gsap.quickTo(tilt, "rotationX", { duration: 0.4, ease: "power3.out" });
  const rotY = gsap.quickTo(tilt, "rotationY", { duration: 0.4, ease: "power3.out" });
  ...
  // KNOCK AROUND
  const items = gsap.utils.toArray(".knockaround li");
...
gsap.to(".img-track", {
  ...
  gsap.to(".expand", {
    scrollTrigger: {
      trigger: ".expand",
      start: "top center",   // when top of element hits middle of viewport
      end: "bottom center",  // remove when it scrolls past
      toggleClass: { targets: ".expand", className: "active" }
    }
  });

  gsap.utils.toArray(".phase *").forEach((el, i) => {

    gsap.to(el, {
      scale: 1.03,
      filter: "blur(0.8px)",
      textShadow: "0 0 10px rgba(162,178,69,0.6)",
      duration: 1.8,
      yoyo: true,
      repeat: -1,
      ease: "sine.inOut",
